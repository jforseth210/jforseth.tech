...
class VideoTestCase(unittest.TestCase):
    ...
    def test_delete_video(self):
        with app.test_client() as tester: 
            ...
            response = tester.post('/videos/deletion', data=dict(youtube_id=video), follow_redirects=True)
            self.assertIs(200, response.status_code)
    ...
    def test_bad_deletion(self):
        ...
        with tester:
            login(tester)
            response = tester.post('/videos/deletion', data=dict(youtube_id="Too short"), follow_redirects=True)
        self.assertIn("This doesn't look like a YouTube link. Try again.", html.unescape(str(response.data)))
    ...
    def test_move_up(self):
        ...
        with app.test_client() as tester:
            login(tester)
            response = tester.post('/videos/move', data=dict(element=video, direction="up"))
    ...
    def test_move_down(self):
        ...
        with app.test_client() as tester:
            login(tester)
            response = tester.post('/videos/move', data=dict(element=video, direction="down"))
